...
def process_mask(mask, bus, bus_route, path, route_name):
    ...
    first_stop_time = None  # 1번 정류장의 시간을 기록하기 위한 변수
    last_first_stop_time = None  # 마지막 1번 정류장의 시간을 기록하기 위한 변수

    start_recording = False
    corrected_mask = np.zeros(len(sorted_mask), dtype=int)  # 보정된 정류장 번호를 저장할 배열

    for i in range(len(sorted_mask)):
        current_time = sorted_bus['Parsed_Date'].iloc[i]
        
        if not start_recording:
            if int(sorted_mask[i]) == 1:
                start_recording = True
                first_stop_time = current_time  # 1번 정류장의 시간 기록
                prev_time = current_time  # prev_time 초기화
                
                # 1번 정류장을 temp_result에 추가
                current = bus_route.iloc[int(sorted_mask[i]) - 1]
        ...
        if current_time > limit_time:
            ...
            if len(temp_result) > save_threshold:
                ...
                num += 1
                save_result(result, path, route_name, sorted_bus, sorted_mask, num)
            # 임시 결과 초기화
            temp_result = pd.DataFrame(columns=columns)
            temp_before_ord = 1  # 1번 정류장부터 시작
        ...
        if current_ord > temp_before_ord and current_ord <= temp_before_ord + 3:
            ...
            current = bus_route.iloc[current_ord - 1]
            corrected_mask[i] = current_ord  # 보정되지 않은 정류장 번호 저장
            ...
            if corrected_ord > temp_before_ord and corrected_ord <= temp_before_ord + 3:
                ...
                corrected_mask[i] = corrected_ord  # 보정된 정류장 번호 저장
                temp_before_ord = current_ord
            else:
        ...
        # 1번 정류장의 시간 수정
        if current_ord == 1:
            last_first_stop_time = current_time  # 마지막 1번 정류장의 시간 갱신
        elif current_ord > 1 and current_ord <= 3 and last_first_stop_time is not None:
            current_time = last_first_stop_time  # 1번 정류장의 시간을 사용
