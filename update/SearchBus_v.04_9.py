...
def devide_up_down(bus_route):
    num = 0
    bus_route['MASK_SELECTED'] = range(1, len(bus_route) + 1)
    for i in range(1, len(bus_route)):
        if bus_route.at[i, 'STOP_ORD'] < bus_route.at[i - 1, 'STOP_ORD']:
            num = bus_route.at[i, 'MASK_SELECTED']
        break  # 첫 번째로 찾은 값만 가져오고 싶다면 break
    return num
    ...
    for i in range(len(sorted_mask)):
        ...
        if current_time > limit_time:
            ...
            if len(temp_result) > save_threshold:
                num += 1
                save_result(temp_result, path, route_name, sorted_bus, num)
            # 임시 결과 초기화
            temp_result = pd.DataFrame(columns=columns)
            temp_before_ord = 1  # 1번 정류장부터 시작
        ...
        # 30번 정류장 도착 전 29번 정류장을 먼저 저장
        if corrected_ord >= 30 and last_28st_stop:
            # current_time과 last_1st_stop의 시간을 비교하여 10분 이상 차이 나는 경우 건너뜀
            time_diff = current_time - last_28st_stop[1]
            if time_diff <= timedelta(minutes=10):
                last_stop = bus_route.iloc[0]
                last_stop['LAT'] = last_28st_stop[0]['LAT']
                last_stop['LNG'] = last_28st_stop[0]['LNG']
                last_stop['Information_Occurrence'] = last_28st_stop[1]
                last_stop['MASK_SELECTED'] = 29  # 1번 정류장의 MASK_SELECTED 값 추가
                temp_result = pd.concat([temp_result, last_stop.to_frame().T])
