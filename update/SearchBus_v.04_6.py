...
SearchBus_v.04_6.py
수정내역: 
1. 1번 및 28번 파일에 저장될 수 있게 수정
2. csv파일 운행당 하나만 생기게 수정
"""
...
def process_mask(mask, bus, bus_route, path, route_name):
    ...
    last_28th_stop = None  # 마지막으로 저장된 28번 정류장 정보
    ...
    for i in range(len(sorted_mask)):
        ...
        if current_ord == 1:
            ...
            last_28th_stop = (sorted_bus.iloc[i], current_time)  # 28번 정류장 정보 저장

        # 2번 정류장 도착 전 1번 정류장을 먼저 저장
        if current_ord == 2 and last_1st_stop:
        ...
        if current_ord == 57:
            ...
            last_28th_stop = None  # 28번 정류장 정보 초기화
